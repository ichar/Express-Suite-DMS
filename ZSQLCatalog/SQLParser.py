...
PythonBooleanTypes    = ( BooleanType, )
...
default_root_columns  = { \
    ...
    'uid'             : ( 'VARCHAR',  500, 'CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL', '', 'UNIQUE INDEX (%s(500))', ),
    'path'            : ( 'VARCHAR',  500, 'CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL', '', 'INDEX (%s(500))',        ),
    'rDate'           : ( 'TIMESTAMP',  0, '',                  '', '',                           ),
}
...
valid_operators       = ( 'AND','OR','AND NOT','OR NOT','NOT', )
list_operators        = ( 'AND','OR','AND NOT','OR NOT', )
...
def init_root( context, name ):
    ...
    sp_drop_catalog_tables = drop( context )
    ...
    return \
        ...
        sp_drop_catalog_tables + end_of_query + \
...
def drop( context ):
    # Returns query for removing of all ZSQLCatalog tables
    # ----------------------------------------------------
    prefix = getPrefix( context )
    ...
    ids = ', '.join(items)
    sp_drop_catalog_tables = ids and 'DROP TABLES %s' % ids or ''
...
def add_extension( context, name, ext_type, ext_spec, as_root=None, default_value=None, engine=None, \
    ...
    if not indexable:
        ...
        indexes = getIndexSpec( as_root, extension_name, name=name, type=type, size=size )
        value[INDEX] = ', '.join([ 'INDEX %s%s%s%s' % ( \
            prefix, name, i or '', indexes[i] ) for i in range(len(indexes)) ])
    elif indexable == 2:
        value[INDEX] = 'FULLTEXT INDEX %s%s(attrValue)' % ( prefix, name )
...
def add_index( context, name, ext_type, ext_spec, as_root=None, default_value=None, engine=None, \
    ...
    if not rs:
        return add_extension( context, name, ext_type, ext_spec, as_root, default_value, indexable=indexable or 1, \
            engine=engine )
    ...
    indexes = getIndexSpec( alter, extension_name, name, size=extSize )
    sp_create_index = \
        end_of_query.join([ 'CREATE INDEX %s%s%s ON %s %s' % ( \
            prefix, name, i or '', extTable, indexes[i] ) for i in range(len(indexes)) ])

    # Try to run action before
    context.run( sp_create_index, no_action=1 )

    idxs = [( name, prefix, extension_name, extTable, ext_spec, engine )]
...
def index( context, key, query=None, sort_by_path=None, distinct=None, limit=None, offset=None ):
    ...
    f, where = parseQuery( context, root, query, f, w )
...
def size( context, query=None ):
    ...
    if query:
        f, where = parseQuery( context, root, query )
        query = \
...
def parseQuery( context, root, query, _from=None, _where=None ):
    ...
    def x_value( value, c ):
        ...
        #
        t = type(value)
        if t in PythonBooleanTypes:
            pass
        elif not value and t not in PythonNumericTypes:
            return c + c
        if t in PythonListTypes:
            return ', '.join([ c + str(i) + c for i in value ])
        if t is type(DateTime()):
            v = value.strftime(datetime_format)
            v = re.sub(r'00\:00\:00', '', v)
        else:
            v = checkQuote(value)
        return c + v.strip() + c

    def x_range( value, range ):
        #
        # Parse attribute range: values list, 'min-max' range
        #
        r = ''
    ...
    def x_keyword( value, mask, a, c ):
        #
        # Parse keyword attribute dictionary
        #
        op = validOperator( value.get('operator') )
        query = ''
        for key, v in value.items():
            if type(v) in PythonDictTypes:
                sw = x_query('%s.attrValue' % a, v, '(%s.attrName=\'%s\' and %s)' % ( a, key, '%s' ), a, c )
            else:
                sw = ''
                if type(v) not in PythonListTypes:
                    ...
                    values = v
                for x in values:
                    r, attr_range = x_range(x, 'eq')
                    sw += '(' + '%s.attrName=\'%s\'' % ( a, key ) + ' and %s.attrValue' % a + r % x_value(x, c) + ')'
            query += (query and ' %s ' % op or '') + mask % sw
        return query
    ...
    def x_scalar( values ):
        ...
        #
        for v in values:
            if type(v) in PythonListTypes:
                return 0
        return 1

    def x_query( key, value, mask, a, c, subquery=None ):
        #
        # Parse query like: { 'query':values[,'range':<'min'|'max'|'min:max'][,'operator':<'AND'|'OR'|'NOT'>] }
        #
        values = value.get('query') or ()
        range = (value.get('range') or 'eq').lower()
        op = validOperator( value.get('operator') )
        ...
        # Filter between subqueries and values
        queries = filter( lambda x: type(x)==type({}), values )
        values = filter( lambda x, queries=queries: x not in queries, values )

        for v in queries:
            # Parse query
            if ext_type == 2:
                # KeywordType index
                sw = x_keyword( v, mask, a, c )
            else:
                # Search query
                sw = x_query( key, v, mask, a, c )
            query += (n and ' %s ' % op or '') + mask % sw
            n += 1

        # Check scalar and not values range
        if range == 'eq' and x_scalar( values ) and op in list_operators:
            values = [ values ]

        for v in values:
            if not ( type(v) in PythonBooleanTypes or v ):
                continue
            # Parse value: ListType or SimpleType index
            r, range = x_range(v, range)
            sw = key + r % x_value(v, c)
            query += (n and ' %s ' % op or '') + mask % sw
            n += 1

        # Join together
        if query and subquery:
            if n > 1: query = '(%s)' % query
            if op == 'NOT': query = 'NOT %s' % query
        return query
    ...
    for x in query:
        ...
        if t in PythonDictTypes:
            # attr query is a dictionary
            subq = 'select distinct RID from %s where' % a
            sq = x_query(key, v, '%s IN (%s %s)' % ( rid_name, subq, '%s' ), a, c, subquery=1)
            a = root

        elif t in PythonListTypes:
            # attr query is a list(tuple)
            x = [ x_value(x, c) for x in v ]
            if x:
                sq = key + ' IN (%s)' % ','.join(x)

        elif t in PythonStringTypes:
            # attr query is a simple string type
            if re.compile(r'[^%s]+' % re.escape('*-+<>()[]~%_'), re.LOCALE).search(v) is None:
                sq = key + ' LIKE ' + x_value(v, c)
            elif hasFullTextIndex:
                sq = 'MATCH(%s) AGAINST(%s IN BOOLEAN MODE)' % ( key, x_value(v, "'").replace('%', '') )
            elif v.find('%') > -1:
                sq = key + ' LIKE ' + x_value(v, c)
            else:
                sq = key + '=' + x_value(v, c)

        elif t in PythonBooleanTypes:
            # attr query is a boolean type: { True | False }
            sq = key + '=' + x_value(v, '')
...
def parse_order_by( context, root, key, reverse=None, _from=None, _where=None ):
    ...
    if a != root:
        if a not in [ x.strip() for x in f.split(',') ]:
            f += ', %s' % a
        w = add_where_item( w, '%s=%s.RID' % ( rid_name, a ) )
...
def getIndexSpec( alter, extension, name=None, type=None, size=None ):
    ...
    if extension == 'KeywordType':
        a('(%s, %s%s)' % ( attr_name, attr_value, size ))
        a('(%s)' % attr_value)
    elif alter:
        size = size and size > 100 and '(100)' or ''
        a('(%s%s)' % ( name, size ))
    else:
        a('(%s%s)' % ( attr_value, size ))
    return x
...
def add_where_item( w, item ):
    if not item or item in w:
        return w
    if w:
        # check if item starts with 'AND' or 'OR'
        if re.match(r'\A\s+(and|or)(?i)', item) is not None:
            return w + item
        return w + ' %s ' % validOperator() + item
    return item
