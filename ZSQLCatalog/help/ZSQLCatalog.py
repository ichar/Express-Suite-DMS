...
def manage_addZSQLCatalog(id, title, vocab_id=None):
...
class ZSQLCatalog:
    ...
    A ZSQLCatalog contains arbitrary index like references to Zope
    objects.  ZSQLCatalog's can index object attribute using a variety
    ...
    The ZSQLCatalog can maintain a table of extra data about cataloged
    ...
    ZSQLCatalog Result objects.  The objects have the same attributes
    as the column of the meta-data table.

    ZSQLCatalog does not store references to the objects themselves, but
    rather to a unique identifier that defines how to get to the
    object.  In Zope, this unique identifier is the object's relative
    path to the ZSQLCatalog (since two Zope objects cannot have the same
    ...
    def search( query_request, sort_index=None, reverse=0, limit=None, merge=1 ):
        ...
        ZSQLCatalog will use a different sort algorithm that uses much less memory
